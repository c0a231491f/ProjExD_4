...
class NeoBeam():
    ...
    def gen_beams(self):
        ...
        for step in range(-50,+51,int(100/(self.num-1))):
          lst.append(Beam(self.bird,step))
...
class EMP(pg.sprite.Sprite):
    ...
    def __init__(self, emys: pg.sprite.Group, bombs: pg.sprite.Group, screen: pg.Surface):
        ...
        self.rect = self.image.get_rect()
        self.life = 3  # 0.05秒表示 * 3フレーム
        self.emys = emys
        self.bombs = bombs

        # 敵機を無効化
        for emy in emys:
            emy.interval = float('inf')  # 爆弾投下を停止
            emy.image = pg.transform.laplacian(emy.image)  # 見た目を変更

        # 爆弾を無効化
        for bomb in bombs:
            bomb.speed *= 0.5  # 速度を半減
            bomb.state = "inactive" #起爆しない
...
def main():
    ...
    while True:
        ...
        left_shift_number = 4097  # 左shiftキーが押されたときにmodsに出力される番号(MSI推奨PCでの環境)
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN and mods == left_shift_number and event.key == pg.K_SPACE:
                ...
                beams.add(Beam(bird))
            if event.type == pg.KEYDOWN and event.key == pg.K_0  and score.value >= 200:
                gravity = Gravity(400)
                gravity_group.add(gravity)
                score.value -= 200
            if key_lst[pg.K_e] and score.value >= 20:
                score.value -= 20  # スコアを消費
                exps.add(EMP(emys, bombs, screen))  # EMP発動
        ...
        if score.value >= 100 and event.type == pg.KEYDOWN and event.key == pg.K_RSHIFT:#右シフトを押したら無敵モード
            bird.hyper_life = 500
            bird.state = "hyper"
            score.value -= 100

        if bird.state == "hyper":#無敵モード状態の設定
        ...
        for bomb in pg.sprite.spritecollide(bird, bombs, True):  # こうかとんと衝突した爆弾リスト
            if bird.state == "normal" and bomb.state == "active": #通常モードの場合
